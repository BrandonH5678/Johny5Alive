"""
MySQLAgent - MySQL database query and extraction
"""
...
class MySQLAgent(BaseAgent):
    ...
    def _check_mysql(self):
        ...
        try:
            ...
            logger.info("mysql-connector-python available")
        except ImportError:
            self.mysql = None
    ...
    def supports(self, target: Any) -> bool:
        """Check if target is a MySQL operation"""
        if not isinstance(target, dict):
            return False

        target_type = target.get('type', '').lower()
        return target_type in ('mysql', 'mariadb')
    ...
    def _list_tables(self, conn, target: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for row in cursor.fetchall():
                ...
                table_type = status[1] if status else 'BASE TABLE'
